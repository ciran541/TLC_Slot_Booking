/**
 * TLC Lead Sync — Google Apps Script
 * ──────────────────────────────────────────────────────────────────────────────
 * Sheet: "Fb"
 * Column X (24) = email
 * Column Y (25) = full_name
 * Column Z (26) = phone_number  (format coming in: "p:+6592296875")
 * ──────────────────────────────────────────────────────────────────────────────
 */
...
var TLC_SYNC_CONFIG = {
  ...
  SHEET_NAME:   "Fb",
  COL_EMAIL:    24,  // Column X
  COL_NAME:     25,  // Column Y
  COL_PHONE:    26,  // Column Z
};
// ─────────────────────────────────────────────────────────────────────────────
...
function onLeadAdded(e) {
  try {
    ...
    if (!sheet) {
      Logger.log("[TLC] Sheet not found: " + TLC_SYNC_CONFIG.SHEET_NAME);
      return;
    }
    ...
    // Read the full row (columns 1→Z = 1→26)
...
function testSyncLastRow() {
  var sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(TLC_SYNC_CONFIG.SHEET_NAME);
  if (!sheet) {
    Logger.log("[TLC] Sheet not found: " + TLC_SYNC_CONFIG.SHEET_NAME);
    return;
  }
